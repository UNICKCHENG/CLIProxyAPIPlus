...
// toolRunRegistry tracks parked tool-call runs by agent id and by exposed call id.
type toolRunRegistry struct {
	mu              sync.Mutex
	byAgentID       map[string]*toolRun
	byExposedCallID map[string]*toolRun
}

func (t *toolRunRegistry) initLocked() {
	if t.byAgentID == nil {
		t.byAgentID = make(map[string]*toolRun)
	}
	if t.byExposedCallID == nil {
		t.byExposedCallID = make(map[string]*toolRun)
	}
}
...
// registry returns the runtime-owned tool run registry the callback server consults.
func (s toolCallbackServer) registry() *toolRunRegistry {
	if s.runtime == nil {
		return &toolRunRegistry{}
	}
	return &s.runtime.toolRuns
}
...
func (r *Runtime) startToolRun(ctx context.Context, req generateRequest, onDelta func(string) error) (generateResult, error) {
	process, errProcess := req.runtime.acquireBridge(req.proxyURL)
	if errProcess != nil {
	...
	agentID, errCreate := r.createAgent(ctx, process, req)
	if errCreate != nil {
...
func (r *Runtime) resumeToolRun(ctx context.Context, req generateRequest, onDelta func(string) error) (generateResult, bool, error) {
	run, resumed, errLookup := r.lookupResumeRun(req)
	if !resumed {
...
func (r *Runtime) lookupResumeRun(req generateRequest) (*toolRun, bool, error) {
	...
	hits := 0
	for _, id := range ids {
		found := r.toolRuns.byExposedCallID[id]
		if found == nil {
			continue
		}
		hits++
...
func (r *Runtime) registerToolRun(run *toolRun) {
	r.toolRuns.mu.Lock()
	r.toolRuns.byAgentID[run.agentID] = run
	r.toolRuns.mu.Unlock()
}
...
// registry returns the runtime-owned tool run registry this run participates in.
func (r *toolRun) registry() *toolRunRegistry {
	if r == nil || r.runtime == nil {
		return &toolRunRegistry{}
	}
	return &r.runtime.toolRuns
}
...
func (r *toolRun) announceBatch() {
	...
	for _, rec := range batch {
		registry.byExposedCallID[rec.exposedID] = r
	}
	registry.mu.Unlock()
...
func (r *toolRun) deliver(results []chatToolResult) error {
	...
	for _, rec := range announced {
		if registry.byExposedCallID[rec.exposedID] == r {
			delete(registry.byExposedCallID, rec.exposedID)
		}
	}
	registry.mu.Unlock()
...
func (r *toolRun) unregister() {
	...
	registry := r.registry()
	registry.mu.Lock()
	if registry.byAgentID[agentID] == r {
		delete(registry.byAgentID, agentID)
	}
	for _, id := range ids {
		if registry.byExposedCallID[id] == r {
			delete(registry.byExposedCallID, id)
		}
	}
	registry.mu.Unlock()
}

func (r *Runtime) abortParkedToolRuns(err error) {
	if err == nil {
		err = errors.New("tool run aborted")
	}
	var runs []*toolRun
	r.toolRuns.mu.Lock()
	for _, run := range r.toolRuns.byAgentID {
...
func (r *Runtime) failToolRunsForProcess(p *bridgeProcess) {
	if p == nil {
		return
	}
	var runs []*toolRun
...
func (s toolCallbackServer) CallCustomTool(ctx context.Context, req *connect.Request[sdkv1.CallCustomToolRequest]) (*connect.Response[sdkv1.CallCustomToolResponse], error) {
	agentID := strings.TrimSpace(req.Msg.GetAgentId())
	name := strings.TrimSpace(req.Msg.GetToolName())

	registry := s.registry()
	registry.mu.Lock()
	run := registry.byAgentID[agentID]
	registry.mu.Unlock()
	if run == nil {
