...
func (r *Runtime) runGenerate(ctx context.Context, req generateRequest, onDelta func(string) error) (generateResult, error) {
	if len(req.toolResults) > 0 {
		result, resumed, errResume := req.runtime.resumeToolRun(ctx, req, onDelta)
		if resumed {
			return result, errResume
		}
	}
	if len(req.tools) == 0 {
		return r.runOneShot(ctx, req, onDelta)
	}
	return r.startToolRun(ctx, req, onDelta)
}

func (r *Runtime) runOneShot(ctx context.Context, req generateRequest, onDelta func(string) error) (generateResult, error) {
	process, errProcess := r.acquireBridge(req.proxyURL)
	if errProcess != nil {
		return generateResult{}, errProcess
	}

	if entry, increment, incrementImages, ok := r.occupySession(req); ok {
		if errBudget := checkPromptBudget(increment); errBudget != nil {
			r.releaseSession(entry, true)
			return generateResult{}, errBudget
		}
		images, errImages := buildSdkImages(ctx, incrementImages, downloadImage)
		if errImages != nil {
			r.releaseSession(entry, false)
			return generateResult{}, errImages
		}
		incrementReq := req
		incrementReq.prompt = increment
		result, errSend := r.sendAndConsume(ctx, entry.process, entry.agentID, incrementReq, images, onDelta)
		if errSend != nil {
			r.releaseSession(entry, false)
			return generateResult{}, errSend
		}
		r.rekeySession(entry, req, result.text)
		return result, nil
	}
	...
	result, errRun := r.sendAndConsume(ctx, process, agentID, req, images, onDelta)
	if errRun != nil {
		releaseAgent(process, agentID, req.apiKey)
		return generateResult{}, errRun
	}
	if !r.rememberSession(req, process, agentID, result.text) {
		releaseAgent(process, agentID, req.apiKey)
	}
	return result, nil
}

func (r *Runtime) sendAndConsume(ctx context.Context, process *bridgeProcess, agentID string, req generateRequest, images []*sdkv1.SdkImage, onDelta func(string) error) (generateResult, error) {
	stream, errSend := openSend(ctx, process, agentID, req, images, onDelta != nil)
	if errSend != nil && isAgentNotFound(errSend) {
		if errResume := r.resumeParkedAgent(ctx, process, agentID, req); errResume == nil {
			stream, errSend = openSend(ctx, process, agentID, req, images, onDelta != nil)
		}
	}
	if errSend != nil {
...
func (r *Runtime) createAgent(ctx context.Context, process *bridgeProcess, req generateRequest) (string, error) {
	// Catalog failures are advisory: an unknown model is forwarded as-is and Cursor decides.
	models, errCatalog := r.catalogFor(ctx, process, req.apiKey, false)
	if errCatalog != nil {
...
func (r *Runtime) resumeParkedAgent(ctx context.Context, process *bridgeProcess, agentID string, req generateRequest) error {
	models, _ := r.catalogFor(ctx, process, req.apiKey, false)
