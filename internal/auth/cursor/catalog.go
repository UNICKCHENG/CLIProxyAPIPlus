...
func (r *Runtime) evictStaleCatalogs() {
	now := time.Now()
	r.catalogs.Range(func(key, value any) bool {
		entry, ok := value.(catalogEntry)
		if !ok || now.Sub(entry.fetched) > catalogTTL {
			r.catalogs.Delete(key)
		}
		return true
	})
}
...
func (r *Runtime) catalogFor(ctx context.Context, process *bridgeProcess, apiKey string, refresh bool) ([]*sdkv1.SdkModel, error) {
	key := catalogCacheKey(apiKey)
	if !refresh {
		if cached, ok := r.catalogs.Load(key); ok {
	...
	r.catalogs.Store(key, catalogEntry{models: models, fetched: time.Now()})
	r.evictStaleCatalogs()
	return models, nil
}
